...
import { signal, computed, batch, nextTick, flushSync } from "../src/reactivity/index.ts";
import { mount } from "../src/renderer/mount.ts";
import { createComponent } from "../src/component/create.ts";
import { onMounted, onUnmounted } from "../src/component/lifecycle.ts";
import { Show, For, Portal, Index, Dynamic } from "../src/component/control-flow.ts";
...
describe("Renderer — Stress Tests (Massive Scale)", () => {
  it("should handle mounting 20,000 intrinsic elements", () => {
    ...
    expect(container.querySelector(".item-19999")?.textContent).toBe("Item 19999");
  });
  ...
  it("should handle extreme component nesting (1,000 levels)", () => {
    const Leaf = createComponent(() => el("span", { id: "leaf" }, "Leaf"));
    ...
    const App = createComponent(() => Nest(1000));
    mount(App, container);
  ...
  it("should handle large-scale reactive prop diffusion", async () => {
    ...
    const Item = createComponent(({ val }: { val: number | (() => number) }) => {
    ...
    expect(container.querySelectorAll(".prop-item")[999].textContent).toBe("1009");
  });
      ...
      const rows = Array.from({ length: 100 }, (_, i) => 
        el("tr", { class: "row" }, 
          Array.from({ length: 100 }, (_, j) => 
            el("td", { class: "cell", "data-pos": `${i}-${j}` }, `Cell ${i},${j}`)
          )
        )
      );
  ...
  it("should handle massive attribute churn", async () => {
    ...
    const App = createComponent(() => {
      return el("div", { 
        id: "target",
        class: () => active.value ? "active massive-class-list-item-1 massive-class-list-item-2" : "inactive",
        "data-active": active,
        title: () => active.value ? "Active State" : "Inactive State",
        style: () => active.value ? "color: red; background: blue; border: 1px solid black;" : "color: black;"
      }, "Content");
    });
  ...
  it("should handle deep fragment nesting (100 layers)", () => {
    ...
    const App = createComponent(() => DeepFragment(100) as SinwanElement);
    mount(App, container);
  ...
  it("should handle large list of conditional components", async () => {
    ...
    const Item = createComponent(({ id, type }: { id: number, type: string }) => {
  ...
  it("should handle massive style object updates", async () => {
    ...
    const App = createComponent(() => el("div", { id: "styled", style: styles }, "Text"));
  ...
  it("should handle component props reactivity across many levels", async () => {
    ...
    const Level3 = createComponent(({ value }: any) => el("span", { id: "result" }, value));
    const Level2 = createComponent(({ value }: any) => el(Level3, { value }));
    const Level1 = createComponent(({ value }: any) => el(Level2, { value }));

    const App = createComponent(() => el(Level1, { value: count }));
    mount(App, container);
  ...
  it("should handle large number of parallel mountings", () => {
    const MountTest = createComponent(({ id }: any) => {
      const inner = signal(0);
      return el("div", { id: `mount-${id}` }, inner);
    });

    const App = createComponent(() => {
      return el("div", {}, Array.from({ length: 500 }, (_, i) => el(MountTest, { id: i })));
    });
  ...
  it("should handle mixed children types (signals, strings, elements, arrays)", async () => {
    const s1 = signal("A");
    const s2 = signal("B");
    const App = createComponent(() => el("div", { id: "mixed" }, 
      s1,
      " static ",
      el("span", {}, s2),
      [el("p", {}, "1"), "2", s1]
  ...
  it("should handle rapid toggling of deeply nested structures", async () => {
    ...
    const App = createComponent(() => {
      return el(Show, {
        when: visible,
        children: () => el("div", { class: "outer" }, 
          Array.from({ length: 10 }, (_, i) => el("div", { class: "inner" }, 
            Array.from({ length: 10 }, (_, j) => el("span", {}, `S${i}-${j}`))
          ))
        )
      });
    });
  ...
  it("should handle extreme recursion in functional components", () => {
    const Recursive = (props: { n: number }): SinwanNode => {
      ...
      return el("div", {}, `Level ${props.n}`, el(Recursive, { n: props.n - 1 }));
    };

    const App = createComponent(() => el(Recursive, { n: 200 }));
    mount(App, container);
  ...
  it("should handle massive batch updates to thousands of nodes", async () => {
    ...
    const App = createComponent(() => el("div", {}, 
  ...
  it("should handle massive child reordering without keys (Index component)", async () => {
    ...
    flushSync();
    expect(container.querySelectorAll(".index-item")[0].textContent).toBe("499");
    expect(container.querySelectorAll(".index-item")[499].textContent).toBe("0");
  });
  ...
  it("should handle concurrent mount/unmount in multiple portals", async () => {
    ...
    const App = createComponent(() => el("div", {}, 
      ...
      el(Show, {
        when: show,
        children: () => [
          el(Portal, { mount: () => doc.getElementById("p1"), children: el("div", { id: "c1" }) }),
          el(Portal, { mount: () => doc.getElementById("p2"), children: el("div", { id: "c2" }) })
        ]
      })
    ));
  ...
  it("should handle massive SVG path stress (500 animated paths)", async () => {
    ...
    const App = createComponent(() => el("svg", {}, 
      Array.from({ length: 500 }, () => el("path", { d }))
    ));

    mount(App, container);
    expect(container.querySelector("path")?.getAttribute("d")).toBe("M 0 0 L 10 10");

    d.value = "M 10 10 L 20 20";
    flushSync();
    expect(container.querySelector("path")?.getAttribute("d")).toBe("M 10 10 L 20 20");
  });

  it("should handle mixed keyed and non-keyed children reordering", async () => {
    const data = signal([1, 2, 3]);
    const App = createComponent(() => el("div", {}, 
      el(For, { each: data, key: (i: any) => i, children: (i: any) => el("span", {}, i) }),
      " - ",
  ...
  it("should handle rapid component property churn", async () => {
    ...
    const Child = createComponent((props: any) => el("div", {}, props.v1, ":", props.v2));
    const App = createComponent(() => el(Child, { v1: p1, v2: p2 }));
  ...
  it("should handle deeply nested Dynamic components", async () => {
    ...
    const App = createComponent(() => RecursiveDynamic(50) as SinwanElement);
    mount(App, container);
  ...
  it("should handle nested For loops with varying keys", async () => {
    ...
    const App = createComponent(() => el(For, {
      each: outer,
      children: (o: any) => el("div", { class: "outer" }, 
        el(For, {
          each: inner,
  ...
  it("should handle massive event listener cleanup", async () => {
    const show = signal(true);
    let clicks = 0;
    const App = createComponent(() => el(Show, {
      when: show,
      children: () => el("div", {}, 
        Array.from({ length: 1000 }, () => el("button", { onclick: () => clicks++ }, "Click"))
      )
    }));
  ...
  it("should handle reactive class name diffusion across tree", async () => {
    ...
    const App = createComponent(() => el("div", { class: theme }, 
      Array.from({ length: 10 }, () => el("div", { class: theme }, 
        Array.from({ length: 10 }, () => el("span", { class: theme }, theme))
      ))
    ));
  ...
  it("should handle massive property object churn on custom components", async () => {
    const valA = signal(1);
    const valB = signal(2);
    const Child = createComponent((p: any) => el("div", {}, p.a, "-", p.b));
    const App = createComponent(() => el(Child, { a: valA, b: valB }));
  ...
  it("should handle extremely deep recursive component mounting", () => {
    const Deep = createComponent((props: { n: number }): SinwanElement => {
      if (props.n <= 0) return el("", {}, "End");
      return el("div", { class: "step" }, el(Deep, { n: props.n - 1 }));
    });

    const App = createComponent(() => el(Deep, { n: 100 }));
    mount(App, container);
  ...
  it("should handle large scale mounting of functional components (non-createComponent)", () => {
  ...
  it("should handle reactive style updates with CSS variables", async () => {
    ...
    const App = createComponent(() => el("div", {
      id: "t",
      style: () => ({ "--theme-color": color.value, color: "var(--theme-color)" })
    }));
  ...
  it("should handle extreme event bubbling (100 layers)", () => {
    ...
    const App = createComponent(() => DeepEvent(100) as SinwanElement);
    mount(App, container);
